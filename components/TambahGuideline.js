...
export default class TambahGuideline extends Component {

    static navigationOptions = ({navigation}) => {        
        const { state } = navigation
        return {
            headerTitle: 'Tambah Guideline',
          }
    }
    ...
    submitReg = async () => {                
        if(this.state.selectedHours < 10){
            this.setState({
                selectedHours: '0'+this.state.selectedHours
            })
        }
        if(this.state.selectedMinutes < 10){
            this.setState({
                selectedMinutes: '0'+this.state.selectedMinutes
            })
        }
        this.setState({
            waktu: this.state.selectedHours+":"+this.state.selectedMinutes
        })
        ...
        try{
            await AsyncStorage.getItem('user', (error, result) => {
                ...
                Resource.tambah_guideline(formdata, tokenString)            
                .then((res) => {                                                                                   
                    console.log(res)
    ...
    edit(){
        Alert.alert(
            "",
            "Apakah anda yakin ingin membuat Guideline ?",
            [
              {
                text: "Cancel",
